 ...
 *
 * Copyright (c) 2021 Eugene Syromiatnikov <evgsyr@gmail.com>.
 * All rights reserved.
 *
...
main(int argc, char *argv[])
{
	...
	rc = syscall(__NR_prctl, -1U, (unsigned long) -2U, (unsigned long) -3U,
				      (unsigned long) -4U, (unsigned long) -5U);
#ifdef PIDNS_TRANSLATION
	pidns_print_leader();
	printf("prctl(" XLAT_UNKNOWN(0xffffffff, "PR_???") ", 0xfffffffe"
	       ", 0xfffffffd, 0xfffffffc, 0xfffffffb) = %s\n", sprintrc(rc));
#endif
